...
def show_apa(obj_list_type='city', obj_type='vtorichnoe-zhile'):
    if obj_list_type == 'city':
        selected_items = InCityObject.objects.filter(object_type__slug=obj_type).filter(is_for_sale=True).order_by(
            '-time_create')
        incity_id = InCityObjectType.objects.get(slug=obj_type)

    else:
        selected_items = OutCityObject.objects.filter(object_type__slug=obj_type).filter(is_for_sale=True).order_by(
            '-time_create')
